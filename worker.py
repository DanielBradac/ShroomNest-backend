...
def run_errand(sensor_data, hum_settings, vent_settings, ip_settings, log_manager, errand_per, init_run):
    ...
    try:
        update_humidifier_state(sensor_data, hum_settings, vent_settings, ip_settings, log_manager, errand_per)
    except Exception as e:
        log_manager.log_event("error", "Worker Error - Humidifier", str(e))
        
    try:
        update_fan_state(vent_settings, hum_settings, ip_settings, log_manager, errand_per)
    except Exception as e:
        log_manager.log_event("error", "Worker Error - Fan", str(e))


# Check on humidifier based on its settings and current sensor data
def update_humidifier_state(sensor_data, hum_settings, vent_settings, ip_settings, log_manager, errand_per):

    # Auto run
    if hum_settings.is_automatic():
        if sensor_data.humidity < hum_settings.range_from and not hum_settings.humidifier_on and toggle_on_condition_met("hum", vent_settings, hum_settings):
    ...
    if hum_settings.is_periodic():
        ...
        if (hum_settings.run_time >= hum_settings.run_per or hum_settings.wait_time >= hum_settings.wait_per):
            ...
            if toggle_on and not toggle_on_condition_met("hum", vent_settings, hum_settings):
                return
...
def update_fan_state(vent_settings, hum_settings, ip_settings, log_manager, errand_per):
    # Periodic run
        if vent_settings.is_periodic():
            ...
            if (vent_settings.run_time >= vent_settings.run_per or vent_settings.wait_time >= vent_settings.wait_per):
                ...
                if toggle_on and not toggle_on_condition_met("fan", vent_settings, hum_settings):
                    return
...
# Checks if there is not a cause for delaying toggle on of device
# devices: "hum"/"fan"
def toggle_on_condition_met(device: str, vent_settings, hum_settings):
    if hum_settings.run_with_fan:
        return True
    
    if device == "hum" and vent_settings.fan_on:
        return False
    if device == "fan" and hum_settings.humidifier_on:
        return False
